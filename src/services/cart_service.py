...
class CartService:
    ...
    async def create(self, cart_data: CartAddDTO) -> CartDTO:
        try:
            ...
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail="Cart create error"
            ) from exc
    ...
    async def delete(self, cart_id: int) -> None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Cart with ID {cart_id} delete error",
            ) from exc
    ...
    async def update(self, cart_id: int, cart_data: CartUpdateDTO) -> CartDTO:
        ...
        if cart_data.customer_id is not None:
            ...
            if not customer:
                raise HTTPException(
                    status_code=HTTP_404_NOT_FOUND,
                    detail=f"User with ID {cart_data.customer_id} not found",
                )

        if cart_data.delivery_address_id is not None:
            delivery_address = await self.address_repo.get_by_id(
                cart_data.delivery_address_id
            )
            if not delivery_address:
                raise HTTPException(
                    status_code=HTTP_404_NOT_FOUND,
                    detail=f"Address with ID {cart_data.delivery_address_id} not found",
                )
        ...
        try:
            ...
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST,
                detail=f"Cart with ID {cart_id} update error",
            ) from exc

        return CartDTO.model_validate(cart)

    async def add_product(
        self, cart_id: int, product_id: int, qty: int = 1
    ) -> CartProductDTO:
        cart = await self.cart_repo.get_by_id(cart_id)
        if not cart:
            raise HTTPException(
        ...
        try:
            ...
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST,
                detail=f"Error while add product with ID {product_id} to cart with ID {cart_id}",
            ) from exc
