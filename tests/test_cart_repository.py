...
class TestCartRepository:
    ...
    async def test_create_cart(
        self, cart_repository: CartRepository, cart_data_1: CartAddDTO
    ):
    ...
    async def test_update_cart(
        ...
        new_cart_data = CartUpdateDTO(
            customer_id=user_2.id, delivery_address_id=address_2.id
        )
    ...
    async def test_add_product(
        self, cart_repository: CartRepository, cart_1: CartDTO, product_1: ProductDTO
    ):
    ...
    @pytest.mark.asyncio
    async def test_add_same_product_multiple_times(
        self, cart_repository: CartRepository, cart_1: CartDTO, product_1: ProductDTO
    ):
    ...
    async def test_get_all_carts(
        self, cart_repository: CartRepository, cart_1: CartDTO, cart_2: CartDTO
    ):
